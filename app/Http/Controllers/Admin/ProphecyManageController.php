...
class ProphecyManageController extends Controller
{
    ...
    public function updateOption(Request $request)
    {
        ...
        $totalInvestByOption = BetInvest::where('betoption_id', $request->id)
                ->where('status','!=',2)
                ->sum('invest_amount');
        $totalInvest = BetInvest::where('betquestion_id', $option->question_id)
                ->where('status','!=',2)
                ->sum('invest_amount');
        $match = Match::find($option->match_id);
        event(new UpdateOption($option, $match, $totalInvestByOption, $totalInvest));
        return back();
    }
